...
The participant sees two candidate shapes and an exactly ambiguous target printed on realistic physical stimulus cards. A scripted agent sees a more diagnostic target, then sends **A**, **B**, or **Pass**. The participant makes an initial choice before the signal and a final choice plus confidence judgment after it.
